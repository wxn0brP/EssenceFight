...
export async function game_effect_use(socket: EFSocket, cardPosOrCardId: string, effectId: string, target: string) {
    ...
    const effectRes = await interpretEffect({
        store: {},
        engine,
        userIndex: socket.user._id === engine.state.users[0] ? 0 : 1,
        sourceCardId: cardId,
        cardPos,
        effect,
        targets: [target]
    ...
    if (!effectRes)
        return res.err("04", "Effect failed");
